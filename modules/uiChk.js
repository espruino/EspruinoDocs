...
, chkU: function(_,e,s,p) { var v=!!s; if (e[9][0]!==v) {
      e[9][0]=v; if ((e[0]&1) && _.di) _.chkDu(_,e[3],e[4],e[5],e[6],e[7],e[8],v,1); }
      if (p&&(p=(p=e[10])?p:e[12])&&p(e[2],_.chkG(e),_,e)) _.d(e); }
, chkD: function(f,c,i,x,y,x2,y2,bc,fc,vi,cb,l) { // draw completely incl. label
    var _=_||this,z=_.ma(0,(x2-x-6)/10); // scaler, outer bevel + margin
    _.dv(1,bc,x,y,x2,y2,2,"vs2",_.mf(z*2)); // 'border' + fill w/ same color
    _.chkDu(_,x,y,x2,y2,bc,fc,vi[0],fc!==bc,z); if (l) _.dl(l,x2,y); } // 'inner'+label
, chkDu: function(_,x,y,x2,y2,bc,fc,s,b,c) { // draw 'inner': fill(border if diff color)
    ...
    if (b) _.dv(1,fc,x,y,x2,y2,m,"vs2",p); // makes border + fill
...
, chk: function(_,e,c) { // is in focus untouch (flip) - else any elt and touch event
    _.chkU(_,e,(c)?!e[9][0]:e[9][0],c); }
};
