...
| `0.25.0.0`        | 115200 |`ESP8266WiFi_0v25` | This appears to be the one of the first stable AT firmwares ([download firmware](/files/ESP8266_AT25-SDK112-512k.bin)) |
| `0.40.0.0`        | 115200 |`ESP8266WiFi_0v25` | This comes pre-installed on the AI-thinker modules used for [Espruino WiFi](/WiFi), and works well with Espruino |
| `1.1.0.0`        | 115200 |`ESP8266WiFi_0v25` | We've tested this firmware on Espruino WiFi and it works fine, however it is not compatible with 512kB ESP01 modules ([download firmware](/files/ESP8266_AT_1_5_4.zip)) |
| `1.3.0.0`        | 115200 |`ESP8266WiFi_0v25` | This is a special 'noboot' version of [SDK 2.0.0](https://github.com/espressif/ESP8266_NONOS_SDK/releases/tag/v2.0.0) that will work on even 512kB modules ([download firmware](/files/ESP8266_AT1.3.0.0-512k.bin)) |
| latest           | 115200 |`ESP8266WiFi_0v25` | Espressif switched to GitHub releases for their new firmwares, however they are not all-in-one firmware files and no longer appear to work on 512kB ESP01 modules ([download firmware](https://github.com/espressif/ESP8266_NONOS_SDK/releases)) |
