...
* Take 2 wires and connect one side of each to GND. Connect the first to GPIO0, while connecting and releasing the second to RST (see the diagram under 'Wiring Up' to find which pins are which).
...
The command above assumes you're running Linux. If you're using Windows or MacOS you'll have to change `/dev/ttyACM0` to the name of the Serial port that Espruino is on.
