...
**Note:** This page is on *using an ESP8266 module with an Espruino board*. If you want to run the
Espruino Firmware on an ESP8266 board, see the [Espruino Builds](https://github.com/espruino/EspruinoBuilds) 
GitHub page instead - the ESP8266 port is still under heavy development.
...
| `0.50.0.0`        | 115200 |`ESP8266WiFi_0v25` | This appears to be the newest firmware ([download firmware](http://bbs.espressif.com/viewtopic.php?f=46&t=1123)) |
...
* Connect to Espruino, copy and paste this code into the left-hand side, and disconnect:
...
```Bash
./esptool.py -p /dev/ttyACM0 -b 19200 write_flash 0 your_firmware.bin
```
...
var wifi = require("ESP8266WiFi").connect(Serial2, function(err) {
  //                ^^^^^^^^^^^
  //                Use ESP8266WiFi_0v25 here (and 115200 baud) if you have an ESP8266 with newer firmware
  if (err) throw err;
