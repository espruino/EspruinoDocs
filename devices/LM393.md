...
function start() {
  ...
  watch1=setWatch(function (a){clearWatch(watch2);setTimeout("onWatch(0)",1000);},A2); //clear the other watch
  watch2=setWatch(function (a){clearWatch(watch1);setTimeout("onWatch(1)",1000);},A3); //timeout allows the value to stabilize, since LDRs do not instantly respond to changes
}
