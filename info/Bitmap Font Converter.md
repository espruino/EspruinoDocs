...
<form id="form">
  ...
  <fieldset>
    ...
    <table>
      ...
      <tr>
        <td><label for="txtCharSpacing">Char spacing</label></td>
  ...
  <fieldset>
    ...
    <p>The image you supply should be white, with black characters arranged in a grid, in <a href="https://en.wikipedia.org/wiki/ASCII#Character_set">ASCII order</a> and then all you need to do is enter the first character, and the width/height of each cell in the grid. You can also use a light grey (#CCC) background to signify areas that should be included in the font, but that should not be black (eg to set the width needed for a space).</p>
...
with a First char of <code>0</code>, Char width of <code>6</code> and Char height of <code>6</code> to get a font containing the digits 0-9. 'Char spacing' is the space that will be added between characters.</p>
  </fieldset>
