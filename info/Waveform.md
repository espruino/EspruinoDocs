...
**Note:** before Espruino 2v25 it's not possible to specify a buffer when creating a Waveform, and you must instead do `w = new Waveform(wave.length);w.buffer.set(wave)`.

You can also pass the data in directly. For instance to output a file directly from Storage (as long as your Storage is on-chip):
...
```JS
var wave = E.toArrayBuffer(require("fs").readFile("sound.raw"));
var w = new Waveform(wave);
analogWrite(A4, 0.5, {freq:40000});
w.startOutput(A4,4000);
```
